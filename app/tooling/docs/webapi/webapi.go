...
func Routes(version string) ([]Route, error) {
	dirEntries, err := os.ReadDir("app/services/mold-api/v1/handlers")
	if err != nil {
	...
	for _, entry := range dirEntries {
		...
		files = append(files, struct {
			...
			entry.Name(),
			fmt.Sprintf("app/services/mold-api/v1/handlers/%s/route.go", entry.Name()),
		})
	}
		...
		f := func(n ast.Node) bool {
			...
			for _, stmt := range funcDecl.Body.List {
				...
				routes = append(routes, Route{
					...
					File:     fmt.Sprintf("app/services/mold-api/%s/handlers/%s/%s.go", version, item.group, item.group),
				})
			}
...
func findAppFunctionFromModel(group string, funcName string) (string, bool, error) {
	...
	file, err := parser.ParseFile(fset, "app/services/mold-api/v1/handlers/"+group+"/model.go", nil, parser.ParseComments)
	if err != nil {
...
func findFilters(group string) ([]string, error) {
	...
	file, err := parser.ParseFile(fset, "app/services/mold-api/v1/handlers/"+group+"/filter.go", nil, parser.ParseComments)
	if err != nil {
		return nil, fmt.Errorf("ParseFile: %w", err)
	}
...
func findOrders(group string) ([]string, error) {
	...
	file, err := parser.ParseFile(fset, "app/services/mold-api/v1/handlers/"+group+"/order.go", nil, parser.ParseComments)
	if err != nil {
		return nil, fmt.Errorf("ParseFile: %w", err)
	}
