...
func VaultInit(vaultConfig vault.Config) error {
	...
	log.Println("Creating mold-api policy")

	err = vaultSrv.CreatePolicy(ctx, "mold-api", "secret/data/*", []string{"read", "create", "update"})
	if err != nil {
	...
	log.Printf("Generating mold-api token: %s", vaultConfig.Token)

	// We don't currently save the token because we're always going to specify it.
	err = vaultSrv.CreateToken(ctx, vaultConfig.Token, []string{"mold-api"}, "Mold API")
	if err != nil {
