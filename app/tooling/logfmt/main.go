...
func init() {
	flag.StringVar(&service, "blueprint", "", "filter which blueprint to see")
}
...
func main() {
	...
	for scanner.Scan() {
		...
		// If a blueprint filter was provided, check.
		if service != "" && strings.ToLower(m["blueprint"].(string)) != service {
			continue
		}
		...
		// {"time":"2023-06-01T17:21:11.13704718Z","level":"INFO","msg":"startup","blueprint":"MOLD-API","GOMAXPROCS":1}
		...
		for k, v := range m {
			switch k {
			case "blueprint", "time", "file", "level", "trace_id", "msg":
				continue
			}
